...
class StereoVONet(nn.Module):
    def __init__(self, network=0, intrinsic=True, flowNormFactor=1.0, stereoNormFactor=1.0, poseDepthNormFactor=0.25, down_scale=True, 
        ...
        if network==0: # PSM + PWC
            from physics_atv_deep_stereo_vo.PWC import PWCDCNet as FlowNet
        ...
        if network==1: # 5_5 + PWC
            ...
            from physics_atv_deep_stereo_vo.FlowNet2 import FlowNet2 as FlowNet
            from physics_atv_deep_stereo_vo.StereoNet7 import StereoNet7 as StereoNet
            self.flowNet   = FlowNet(middleblock=3)

        self.stereoNet = StereoNet()

        from physics_atv_deep_stereo_vo.VOFlowNet import VOFlowRes as FlowPoseNet
