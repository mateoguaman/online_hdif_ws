...
class Conv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride = 1, padding=None, bn = False, relu = True):
        super(Conv, self).__init__()
        self.inp_dim = inp_dim
        padding = (kernel_size-1)//2 if padding is None else padding
        self.conv = nn.Conv2d(inp_dim, out_dim, kernel_size, stride, padding=padding, bias=True)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, downsample, pad, dilation):
        ...
        self.conv2 = nn.Conv2d(planes, planes, 3, 1, padding = pad, dilation = dilation, bias=True) #Conv(planes, planes, 3, 1)
...
def make_layer(block, inplanes, planes, blocks, stride, pad, dilation):
    '''
    ...
    for i in range(1, blocks):
        layers.append(block(planes * block.expansion, planes,1,None,pad,dilation))
